%% case 3 effect of comm frequency
...
%% noise generation alpha = 1000 muw = 1e-6
...
%% noise generation  alpha = 300 muw = 3e-6
...
% PCF
Tc = [0.1 0.5 1 3]; % communication interval (seconds)

results = cell(length(Tc), 1); % Initialize a cell array to store results
FedDMCANC_temp = FedDMCANC; % Preserve the original FedDMCANC
for idx = 1:length(Tc)
    [e_FedDMCANC, FedDMCANC_temp] = FedMCANC_166_PCF(FedDMCANC_temp, muw, alpha, Wcsubopt, Tc(idx));
...
% save('Result/Case3','FedDMCANC_ideal','e_FedDMCANC_ideal','results','Ref','Dis','muw','alpha','Tc');
